...
| 26 | **Deep Links** | App Links `autoVerify` configured but `.well-known/assetlinks.json` not verified | **P2** | `AndroidManifest.xml:38`: `android:autoVerify="true"` for `https://windyword.ai/app`. If the Digital Asset Links file isn't hosted properly, verified links will silently fail and show a disambiguation dialog. |
