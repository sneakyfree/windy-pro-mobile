...
| 5b | WebView injection / origin | **P0-1**: `(tabs)/chat.tsx:193` uses `originWhitelist=['https://*','http://*']` and injects `windy_auth_token` into every page. Any redirect out of `chat.windyword.ai` (misconfig or subdomain takeover) leaks the JWT. | P0-1 |
